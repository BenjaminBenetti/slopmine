...
/**
 * Configuration for berry bush patch generation.
 */
export interface BerryBushFeatureSettings {
  /** Density of bush patches (lower = rarer). treeDensity for plains is 3.0, use much lower */
  density: number
  /** Minimum bushes per patch */
  minPatchSize: number
  /** Maximum bushes per patch */
  maxPatchSize: number
  /** Grid size for patch placement (larger = more spread out) */
  gridSize: number
  /** Valid ground blocks for bush placement (defaults to [GRASS]) */
  validGroundBlocks?: number[]
}

/**
 * Berry bush feature that places sparse patches of laden berry bushes
 * (BERRY_BUSH_BERRIES) in grassy clearings, ready for the player to forage.
 * Harvested bushes swap to the picked-clean variant and regrow at runtime.
 */
export class BerryBushFeature extends Feature {
  readonly settings: BerryBushFeatureSettings

  constructor(settings: BerryBushFeatureSettings) {
    super()
  ...
  async scan(context: FeatureContext): Promise<void> {
    ...
    // Check each cell in a grid pattern for potential bush patch positions
    for (let localX = 0; localX < CHUNK_SIZE_X; localX += gridSize) {
      for (let localZ = 0; localZ < CHUNK_SIZE_Z; localZ += gridSize) {
        ...
        const patchChance = this.positionRandom(patchWorldX, patchWorldZ, 800)
        ...
        const bushY = groundHeight + 1 // Place bush on top of the ground

        // Skip if bush Y is outside this sub-chunk
        if (bushY < subChunkMinY || bushY > subChunkMaxY) continue

        // Determine patch size
        const patchSize = minPatchSize + Math.floor(
          this.positionRandom(patchWorldX, patchWorldZ, 803) * (maxPatchSize - minPatchSize + 1)
        )

        // Place bushes in a small cluster around the patch center
        this.placeBushPatch(
          chunk,
          coord,
          patchWorldX,
  ...
  /**
   * Place a small patch of laden berry bushes around a center point.
   */
  private placeBushPatch(
    ...
    // Place bushes in a small area around center (roughly circular)
    const offsets = [
      [0, 0],   // Center
      [1, 0],   // Adjacent
      [-1, 0],
    ...
    for (const [dx, dz] of shuffled) {
      ...
      const localY = bushY - subChunkMinY

      // Check if ground is a valid block for bush placement
      ...
      if (localY >= 0 && localY < SUB_CHUNK_HEIGHT) {
        ...
        if (currentBlock === BlockIds.AIR) {
          // Spawn laden with berries so freshly generated bushes are forageable
          chunk.setBlockId(localX, localY, localZ, BlockIds.BERRY_BUSH_BERRIES)
          placed++
        }
      }
    }
