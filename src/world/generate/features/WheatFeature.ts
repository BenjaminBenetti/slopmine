...
/**
 * Configuration for wheat patch generation.
 */
export interface WheatFeatureSettings {
  /** Density of wheat patches (lower = rarer). treeDensity for plains is 3.0, use much lower */
  density: number
  /** Minimum wheat plants per patch */
  minPatchSize: number
  /** Maximum wheat plants per patch */
  maxPatchSize: number
  /** Grid size for patch placement (larger = more spread out) */
  gridSize: number
  /** Block to replace ground with when placing wheat (e.g., DIRT for farmland effect) */
  soilBlock?: number
}

/**
 * Wheat feature that places small patches of wheat on grass surfaces.
 * Wheat is placed as stage 1 (seedling) and will grow over time via WheatBlockEntity.
 */
export class WheatFeature extends Feature {
  readonly settings: WheatFeatureSettings

  constructor(settings: WheatFeatureSettings) {
    super()
  ...
  async scan(context: FeatureContext): Promise<void> {
    ...
    // Check each cell in a grid pattern for potential wheat patch positions
    for (let localX = 0; localX < CHUNK_SIZE_X; localX += gridSize) {
      for (let localZ = 0; localZ < CHUNK_SIZE_Z; localZ += gridSize) {
        ...
        const jitterX = Math.floor(this.positionRandom(worldX, worldZ, 101) * gridSize)
        const jitterZ = Math.floor(this.positionRandom(worldX, worldZ, 102) * gridSize)

        const patchWorldX = worldX + jitterX
        const patchWorldZ = worldZ + jitterZ

        // Probability check for patch placement
        const patchChance = this.positionRandom(patchWorldX, patchWorldZ, 100)
        ...
        const wheatY = groundHeight + 1 // Place wheat on top of grass

        // Skip if wheat Y is outside this sub-chunk
        if (wheatY < subChunkMinY || wheatY > subChunkMaxY) continue

        // Determine patch size
        const patchSize = minPatchSize + Math.floor(
          this.positionRandom(patchWorldX, patchWorldZ, 103) * (maxPatchSize - minPatchSize + 1)
        )

        // Place wheat in a small cluster around the patch center
        this.placeWheatPatch(
          chunk,
          coord,
          patchWorldX,
          patchWorldZ,
  ...
  /**
   * Place a small patch of wheat around a center point.
   */
  private placeWheatPatch(
    ...
    // Place wheat in a small area around center (roughly circular)
    ...
    for (let i = shuffled.length - 1; i > 0; i--) {
      const j = Math.floor(this.positionRandom(centerWorldX, centerWorldZ, 200 + i) * (i + 1))
    ...
    for (const [dx, dz] of shuffled) {
      ...
      const localY = wheatY - subChunkMinY
      ...
      if (localY >= 0 && localY < SUB_CHUNK_HEIGHT) {
        ...
        if (currentBlock === BlockIds.AIR) {
          // Place wheat
          chunk.setBlockId(localX, localY, localZ, BlockIds.WHEAT_1)
