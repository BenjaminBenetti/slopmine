...
/**
 * Settings for riverbank clay feature.
 */
export interface RiverbankClayFeatureSettings {
  /** Water level Y coordinate */
  readonly waterLevel: number
  /** Maximum height above water to place clay (thin strip) */
  readonly maxHeightAboveWater: number
  /** Maximum depth below surface to place clay */
  readonly maxDepthBelowSurface: number
  /** Large-scale noise frequency for clay zones */
...
/**
 * Riverbank clay feature that places patchy clay blocks along water edges.
 ...
 */
export class RiverbankClayFeature extends Feature {
  readonly settings: RiverbankClayFeatureSettings

  constructor(settings: RiverbankClayFeatureSettings) {
    super()
  ...
  async scan(context: FeatureContext): Promise<void> {
    ...
    for (let localX = 0; localX < CHUNK_SIZE_X; localX++) {
      for (let localZ = 0; localZ < CHUNK_SIZE_Z; localZ++) {
        ...
        // Use offset coordinates to differentiate from mud patches
        const noiseOffsetX = 1000
        const noiseOffsetZ = 1000
        const zoneNoise = noise.noise2D((worldX + noiseOffsetX) * zoneFrequency, (worldZ + noiseOffsetZ) * zoneFrequency)
        const patchNoise = noise.noise2D((worldX + noiseOffsetX) * patchFrequency, (worldZ + noiseOffsetZ) * patchFrequency)
        ...
        for (let worldY = searchStartY; worldY >= searchEndY; worldY--) {
          ...
          if (block === BlockIds.GRASS || block === BlockIds.DIRT || block === BlockIds.STONE || block === BlockIds.MUD) {
            surfaceY = worldY
            break
          }
        }
        ...
        // Place clay from surface down to maxDepthBelowSurface
        ...
        for (let worldY = startY; worldY >= endY; worldY--) {
          ...
          // Replace grass, dirt, stone, or mud with clay
          if (currentBlock === BlockIds.GRASS || currentBlock === BlockIds.DIRT ||
              currentBlock === BlockIds.STONE || currentBlock === BlockIds.MUD) {
  ...
  private positionHash(x: number, y: number, z: number): number {
    // Use different constants than mud to avoid overlap patterns
    const n = Math.sin(x * 17.1234 + y * 7.5678 + z * 91.4321) * 51234.9876
