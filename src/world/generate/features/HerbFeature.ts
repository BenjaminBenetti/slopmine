...
/**
 * Configuration for herb patch generation.
 */
export interface HerbFeatureSettings {
  /** Density of herb patches (lower = rarer). treeDensity for plains is 3.0, use much lower */
  density: number
  /** Minimum herb plants per patch */
  minPatchSize: number
  /** Maximum herb plants per patch */
  maxPatchSize: number
  /** Grid size for patch placement (larger = more spread out) */
  gridSize: number
  /** Valid ground blocks for herb placement (defaults to [GRASS]) */
  validGroundBlocks?: number[]
}

/**
 * Herb feature that places small patches of herbs on grass surfaces.
 * Herbs are placed as stage 1 (seedling) and will grow over time via HerbBlockEntity.
 */
export class HerbFeature extends Feature {
  readonly settings: HerbFeatureSettings

  constructor(settings: HerbFeatureSettings) {
    super()
  ...
  async scan(context: FeatureContext): Promise<void> {
    ...
    // Check each cell in a grid pattern for potential herb patch positions
    for (let localX = 0; localX < CHUNK_SIZE_X; localX += gridSize) {
      for (let localZ = 0; localZ < CHUNK_SIZE_Z; localZ += gridSize) {
        ...
        const jitterX = Math.floor(this.positionRandom(worldX, worldZ, 301) * gridSize)
        const jitterZ = Math.floor(this.positionRandom(worldX, worldZ, 302) * gridSize)

        const patchWorldX = worldX + jitterX
        const patchWorldZ = worldZ + jitterZ

        // Probability check for patch placement
        const patchChance = this.positionRandom(patchWorldX, patchWorldZ, 300)
        ...
        const herbY = groundHeight + 1 // Place herb on top of grass

        // Skip if herb Y is outside this sub-chunk
        if (herbY < subChunkMinY || herbY > subChunkMaxY) continue

        // Determine patch size
        const patchSize = minPatchSize + Math.floor(
          this.positionRandom(patchWorldX, patchWorldZ, 303) * (maxPatchSize - minPatchSize + 1)
        )

        // Place herbs in a small cluster around the patch center
        this.placeHerbPatch(
          chunk,
          coord,
          patchWorldX,
  ...
  /**
   * Place a small patch of herbs around a center point.
   */
  private placeHerbPatch(
    ...
    // Place herbs in a small area around center (roughly circular)
    ...
    for (let i = shuffled.length - 1; i > 0; i--) {
      const j = Math.floor(this.positionRandom(centerWorldX, centerWorldZ, 400 + i) * (i + 1))
    ...
    for (const [dx, dz] of shuffled) {
      ...
      const localY = herbY - subChunkMinY

      // Check if ground is a valid block for herb placement
      ...
      if (localY >= 0 && localY < SUB_CHUNK_HEIGHT) {
        ...
        if (currentBlock === BlockIds.AIR) {
          // Place herb as seedling (stage 1)
          chunk.setBlockId(localX, localY, localZ, BlockIds.HERB_1)
          placed++
        }
      }
    }
