...
registerTextureUrl(TextureId.WATER_HALF, waterTexUrl, true)

const waterTexture = loadBlockTexture(waterTexUrl)

/**
 * Create geometry for 1/2 height water with corrected UVs.
 * Side faces show bottom half of texture to avoid stretching.
 */
function createWaterHalfGeometry(): THREE.BufferGeometry {
  const height = 0.5
  ...
  for (const vertIdx of sideFaceIndices) {
    ...
    // Scale V from 0-1 to 0-0.5 (bottom half of texture)
...
const waterHalfGeometry = createWaterHalfGeometry()

/**
 * Semi-transparent blue material for partial water with texture.
 */
const waterHalfMaterial = new THREE.MeshLambertMaterial({
  map: waterTexture,
  transparent: true,
  opacity: 0.75,
...
/**
 * Water half block - a transparent, non-solid liquid at 1/2 height.
 * Used in the liquid physics system for volume equalization.
 */
export class WaterHalfBlock extends TransparentBlock {
  ...
  protected get defaultTextureId(): number {
    return TextureId.WATER_HALF
  }

  protected getGeometry(): THREE.BufferGeometry {
    return waterHalfGeometry
  }

  protected getMaterials(): THREE.Material {
    return waterHalfMaterial
  }
