...
import redwoodTrapdoorTexUrl from '../redwood_trapdoor/assets/redwood-trapdoor.webp'

// Load texture and create this module's material
const redwoodTrapdoorTexture = loadBlockTexture(redwoodTrapdoorTexUrl)
const redwoodTrapdoorOpenMaterial = new THREE.MeshLambertMaterial({ map: redwoodTrapdoorTexture })
...
const redwoodTrapdoorOpenGeometry = new THREE.BoxGeometry(1, 1, PANEL_THICKNESS)
  .translate(0, 0, -0.5 + PANEL_THICKNESS / 2)

/**
 * Redwood trapdoor (open state) - non-solid vertical panel against the -Z edge.
 * E-key toggles back to REDWOOD_TRAPDOOR via the trapdoor toggle helper
 * (registered centrally in main.ts). Drops the same item as the closed state.
 */
export class RedwoodTrapdoorOpenBlock extends SolidBlock {
  readonly properties: IBlockProperties = {
    id: BlockIds.REDWOOD_TRAPDOOR_OPEN,
    name: 'redwood_trapdoor_open',
    isOpaque: false,
    isSolid: false,
    isLiquid: false,
    hardness: 1.5,
  ...
  protected getGeometry(): THREE.BufferGeometry {
    return redwoodTrapdoorOpenGeometry
  }

  protected getMaterials(): THREE.Material {
    return redwoodTrapdoorOpenMaterial
  }
