...
export class WoodworkingBenchBlock extends SolidBlock {
  ...
  /**
   * Return the bench's slot contents so they go back to the player on break.
  ...
  getStateDrops(x: bigint, y: bigint, z: bigint): ReadonlyArray<{ readonly item: IItem; count: number }> {
    const state = BlockStateManager.getInstance().getState<WoodworkingBenchState>({ x, y, z })
    return state ? state.getAllItems() : []
  }

  /**
   * Remove empty (count <= 0) stacks from the bench's slots after a partial
   * content transfer.
   */
