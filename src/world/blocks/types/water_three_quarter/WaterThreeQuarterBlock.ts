...
registerTextureUrl(TextureId.WATER_THREE_QUARTER, waterTexUrl, true)

const waterTexture = loadBlockTexture(waterTexUrl)

/**
 * Create geometry for 3/4 height water with corrected UVs.
 * Side faces show bottom 3/4 of texture to avoid stretching.
 */
function createWaterThreeQuarterGeometry(): THREE.BufferGeometry {
  const height = 0.75
  const yOffset = -0.125 // Centers the block so bottom is at -0.5

  const geometry = new THREE.BoxGeometry(1, height, 1)
  geometry.translate(0, yOffset, 0)

  // Fix UVs for side faces - show bottom 3/4 of texture (V: 0 to 0.75)
  ...
  // We need to scale V coordinates on sides
  ...
  for (const vertIdx of sideFaceIndices) {
    ...
    // Scale V from 0-1 to 0-0.75 (bottom 3/4 of texture)
...
const waterThreeQuarterGeometry = createWaterThreeQuarterGeometry()

/**
 * Semi-transparent blue material for partial water with texture.
 */
const waterThreeQuarterMaterial = new THREE.MeshLambertMaterial({
  map: waterTexture,
  transparent: true,
  opacity: 0.75,
...
/**
 * Water three-quarter block - a transparent, non-solid liquid at 3/4 height.
 * Used in the liquid physics system for volume equalization.
 */
export class WaterThreeQuarterBlock extends TransparentBlock {
  ...
  protected get defaultTextureId(): number {
    return TextureId.WATER_THREE_QUARTER
  }

  protected getGeometry(): THREE.BufferGeometry {
    return waterThreeQuarterGeometry
  }

  protected getMaterials(): THREE.Material {
    return waterThreeQuarterMaterial
  }
