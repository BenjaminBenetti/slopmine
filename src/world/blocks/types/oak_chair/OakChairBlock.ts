...
// Reuse the oak planks texture (registered by the planks block module)
import oakPlanksTexUrl from '../oak_planks/assets/oak-planks.webp'

const oakPlanksTexture = loadBlockTexture(oakPlanksTexUrl)
const oakChairMaterial = new THREE.MeshLambertMaterial({ map: oakPlanksTexture })
...
/**
 * Oak chair - seat with four legs and a backrest, open toward +Z (front).
 * Uses the oak planks texture. Facing rotation via placement metadata.
 */
export class OakChairBlock extends SolidBlock {
  readonly properties: IBlockProperties = {
    id: BlockIds.OAK_CHAIR,
    name: 'oak_chair',
    isOpaque: false,
    isSolid: true,
    isLiquid: false,
