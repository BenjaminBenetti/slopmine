...
import oakTrapdoorTexUrl from './assets/oak-trapdoor.webp'

// Register texture for atlas
registerTextureUrl(TextureId.OAK_TRAPDOOR, oakTrapdoorTexUrl)

// Load texture and create material
const oakTrapdoorTexture = loadBlockTexture(oakTrapdoorTexUrl)
const oakTrapdoorMaterial = new THREE.MeshLambertMaterial({ map: oakTrapdoorTexture })
...
const oakTrapdoorClosedGeometry = new THREE.BoxGeometry(1, PANEL_THICKNESS, 1)
  .translate(0, -0.5 + PANEL_THICKNESS / 2, 0)

/**
 * Oak trapdoor (closed state) - thin solid panel lying on the cell floor.
 * E-key toggles to OAK_TRAPDOOR_OPEN via the trapdoor toggle helper
 * (registered centrally in main.ts).
 */
export class OakTrapdoorBlock extends SolidBlock {
  readonly properties: IBlockProperties = {
    id: BlockIds.OAK_TRAPDOOR,
    name: 'oak_trapdoor',
    isOpaque: false,
    isSolid: true,
    isLiquid: false,
  ...
  protected getGeometry(): THREE.BufferGeometry {
    return oakTrapdoorClosedGeometry
  }

  protected getMaterials(): THREE.Material {
    return oakTrapdoorMaterial
  }
