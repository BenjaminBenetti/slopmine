...
registerTextureUrl(TextureId.WATER_FIVE_EIGHTH, waterTexUrl, true)

const waterTexture = loadBlockTexture(waterTexUrl)

/**
 * Create geometry for 5/8 height water with corrected UVs.
 * Side faces show bottom 5/8 of texture to avoid stretching.
 */
function createWaterFiveEighthGeometry(): THREE.BufferGeometry {
  const height = 0.625
  ...
  // Fix UVs for side faces - show bottom 5/8 of texture (V: 0 to 0.625)
  ...
  for (const vertIdx of sideFaceIndices) {
    ...
    // Scale V from 0-1 to 0-0.625 (bottom 5/8 of texture)
...
const waterFiveEighthGeometry = createWaterFiveEighthGeometry()

/**
 * Semi-transparent blue material for partial water with texture.
 */
const waterFiveEighthMaterial = new THREE.MeshLambertMaterial({
  map: waterTexture,
  transparent: true,
  opacity: 0.75,
...
/**
 * Water five-eighth block - a transparent, non-solid liquid at 5/8 height.
 * Used in the liquid physics system for smooth volume distribution.
 */
export class WaterFiveEighthBlock extends TransparentBlock {
  ...
  protected get defaultTextureId(): number {
    return TextureId.WATER_FIVE_EIGHTH
  }

  protected getGeometry(): THREE.BufferGeometry {
    return waterFiveEighthGeometry
  }

  protected getMaterials(): THREE.Material {
    return waterFiveEighthMaterial
  }
