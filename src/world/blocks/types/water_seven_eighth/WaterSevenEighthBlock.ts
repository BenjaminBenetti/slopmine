...
registerTextureUrl(TextureId.WATER_SEVEN_EIGHTH, waterTexUrl, true)

const waterTexture = loadBlockTexture(waterTexUrl)

/**
 * Create geometry for 7/8 height water with corrected UVs.
 * Side faces show bottom 7/8 of texture to avoid stretching.
 */
function createWaterSevenEighthGeometry(): THREE.BufferGeometry {
  const height = 0.875
  ...
  // Fix UVs for side faces - show bottom 7/8 of texture (V: 0 to 0.875)
  ...
  for (const vertIdx of sideFaceIndices) {
    ...
    // Scale V from 0-1 to 0-0.875 (bottom 7/8 of texture)
...
const waterSevenEighthGeometry = createWaterSevenEighthGeometry()

/**
 * Semi-transparent blue material for partial water with texture.
 */
const waterSevenEighthMaterial = new THREE.MeshLambertMaterial({
  map: waterTexture,
  transparent: true,
  opacity: 0.8,
...
/**
 * Water seven-eighth block - a transparent, non-solid liquid at 7/8 height.
 * Used in the liquid physics system for smooth volume distribution.
 */
export class WaterSevenEighthBlock extends TransparentBlock {
  ...
  protected get defaultTextureId(): number {
    return TextureId.WATER_SEVEN_EIGHTH
  }

  protected getGeometry(): THREE.BufferGeometry {
    return waterSevenEighthGeometry
  }

  protected getMaterials(): THREE.Material {
    return waterSevenEighthMaterial
  }
