...
// Reuse the redwood planks texture (registered by the redwood planks block module)
...
const redwoodStairsMaterial = new THREE.MeshLambertMaterial({ map: redwoodPlanksTexture })

// Geometry centered around Y=0 (renderer adds +0.5).
// Step ascends toward -Z; front (+Z) is the low side.
const redwoodStairsGeometry = (() => {
...
/**
 * Redwood stairs - two merged boxes forming a step ascending toward -Z.
 * Facing rotation is handled automatically by the non-greedy instanced path.
 */
export class RedwoodStairsBlock extends SolidBlock {
  readonly properties: IBlockProperties = {
    id: BlockIds.REDWOOD_STAIRS,
    name: 'redwood_stairs',
    isOpaque: false,
    isSolid: true,
    isLiquid: false,
  ...
  protected getGeometry(): THREE.BufferGeometry {
    return redwoodStairsGeometry
  }

  protected getMaterials(): THREE.Material {
    return redwoodStairsMaterial
  }
  ...
  getDrops(): IItem[] {
    return [new RedwoodStairsBlockItem()]
  }
}
